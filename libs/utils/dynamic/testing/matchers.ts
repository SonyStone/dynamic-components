...
_global.beforeEach(function() {
  ...
  jasmine.addMatchers({
    toBePromise() {
      return {
        compare(actual: any) {
          ...
          return {pass, get message() { return 'Expected ' + actual + ' to be a promise'; }};
        }
      };
    },
    ...
    toContainComponent() {
      return {
        compare(actualFixture: any, expectedComponentType: Type<any>) {
...
export function elementText(n: any): string {
  ...
  if (getDOM().isElementNode(n) && (n as Element).tagName === 'CONTENT') {
    return elementText(Array.prototype.slice.apply((n as any).getDistributedNodes()));
  }

  if (hasShadowRoot(n)) {
    return elementText(childNodesAsList((n as any).shadowRoot));
  }
