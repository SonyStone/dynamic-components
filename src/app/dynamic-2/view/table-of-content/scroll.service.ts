...
export class ScrollService implements OnDestroy {
  ...
  supportManualScrollRestoration: boolean =
      !! this.window
      && ('scrollTo' in this.window)
      && ('scrollX' in this.window)
  ...
  scrollToElement(element: Element|null) {
    if (element) {
      ...
      if (this.window && this.window.scrollBy) {
        ...
        //  element.)
        this.window.scrollBy(0, element.getBoundingClientRect().top - this.topOffset);

        // If we are very close to the top (<20px), then scroll all the way up.
        // (This can happen if `element` is at the top of the page, but has a small top-margin.)
        if (this.window.pageYOffset < 20) {
          this.window.scrollBy(0, -this.window.pageYOffset);
        }
      }
    }
  }
